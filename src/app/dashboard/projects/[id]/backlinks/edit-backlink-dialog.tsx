...
import { Backlink, BacklinkType } from "@prisma/client"
...
interface Props {
  projectId: string
  backlink: Backlink
  onBacklinkUpdated: () => void
}

export function EditBacklinkDialog({ projectId, backlink, onBacklinkUpdated }: Props) {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      const response = await fetch(`/api/projects/${projectId}/backlinks/${backlink.id}`, {
        method: 'PATCH',
      ...
      if (!response.ok) {
        throw new Error(data.error || 'Failed to update backlink')
      }
      ...
      toast({
        title: "Success",
        description: "Backlink updated successfully",
      })
    } catch (error) {
      console.error('Error updating backlink:', error)
      toast({
        title: "Error",
        description: error instanceof Error ? error.message : "Failed to update backlink",
        variant: "destructive",
      })
    } finally {
      setIsLoading(false)
